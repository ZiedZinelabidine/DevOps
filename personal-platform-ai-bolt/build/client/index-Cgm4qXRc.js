import { L as I, E as c, C as D } from "./index-DPyK7ojY.js";
import { s as L, t as n, B as H, L as B, m as K, a as M, i as OO, f as eO, d as X, b as iO, J as aO, I as nO, F as rO, A as d } from "./Header-DaHJSYVl.js";
const QO = 1, j = 194, Z = 195, tO = 196, x = 197, oO = 198, dO = 199, sO = 200, lO = 2, E = 3, u = 201, TO = 24, SO = 25, pO = 49, qO = 50, gO = 55, PO = 56, mO = 57, $O = 59, cO = 60, hO = 61, XO = 62, fO = 63, yO = 65, zO = 238, WO = 71, RO = 241, vO = 242, kO = 243, _O = 244, xO = 245, uO = 246, UO = 247, bO = 248, w = 72, VO = 249, GO = 250, jO = 251, ZO = 252, EO = 253, wO = 254, YO = 255, FO = 256, CO = 73, JO = 77, AO = 263, NO = 112, IO = 130, DO = 151, LO = 152, HO = 155, p = 10, q = 13, k = 32, h = 9, _ = 35, BO = 40, KO = 46, v = 123, U = 125, Y = 39, F = 34, MO = 92, Oe = 111, ee = 120, ie = 78, ae = 117, ne = 85, re = /* @__PURE__ */ new Set([
  SO,
  pO,
  qO,
  AO,
...
const Qe = new c((O, e) => {
  ...
  if (a == p || a == q) {
    ...
    for (; ; ) {
      if (O.next == k) r++;
      else if (O.next == h) r += 8 - r % 8;
      else break;
      O.advance(), Q++;
    }
    r != i.indent && O.next != p && O.next != q && O.next != _ && (r < i.indent ? O.acceptToken(Z, -Q) : O.acceptToken(j));
  }
...
const b = new Map([
  ...
  [YO, S | T | l],
  [FO, S | T | l | s]
].map(([O, e]) => [O, e | C])), se = new D({
  start: oe,
  reduce(O, e, i, a) {
    return O.flags & m && re.has(e) || (e == WO || e == w) && O.flags & C ? O.parent : O;
  },
  shift(O, e, i, a) {
    return e == j ? new $(O, de(a.read(a.pos, i.pos)), 0) : e == Z ? O.parent : e == TO || e == gO || e == $O || e == E ? new $(O, 0, m) : b.has(e) ? new $(O, 0, b.get(e) | O.flags & m) : O;
  },
  hash(O) {
    return O.hash;
  }
  ...
  if (!/\w/.test(String.fromCharCode(O.next)))
    for (let e = 0; ; e++) {
      let i = O.peek(e);
      if (!(i == k || i == h)) {
        i != BO && i != KO && i != p && i != q && i != _ && O.acceptToken(QO);
        return;
      }
    }
}), Te = new c((O, e) => {
  let { flags: i } = e.context, a = i & s ? F : Y, r = (i & l) > 0, Q = !(i & T), t = (i & S) > 0, o = O.pos;
  for (; !(O.next < 0); )
    if (t && O.next == v)
      if (O.peek(1) == v)
        O.advance(2);
      ...
      if (O.pos == o) {
        O.advance();
        let g = O.next;
        g >= 0 && (O.advance(), Se(O, g)), O.acceptToken(lO);
        return;
      }
      break;
...
function Se(O, e) {
  ...
  else if (e == ie && O.next == v) {
    for (O.advance(); O.next >= 0 && O.next != U && O.next != Y && O.next != F && O.next != p; ) O.advance();
    O.next == U && O.advance();
  }
}
const pe = L({
  ...
  ".": n.derefOperator,
  ", ;": n.separator
}), qe = { __proto__: null, await: 44, or: 54, and: 56, in: 60, not: 62, is: 64, if: 70, else: 72, lambda: 76, yield: 94, from: 96, async: 102, for: 104, None: 162, True: 164, False: 164, del: 178, pass: 182, break: 186, continue: 190, return: 194, raise: 202, import: 206, as: 208, global: 212, nonlocal: 214, assert: 218, type: 223, elif: 236, while: 240, try: 246, except: 248, finally: 250, with: 254, def: 258, class: 268, match: 279, case: 285 }, ge = I.deserialize({
  version: 14,
  ...
  tokenPrec: 7664
}), V = /* @__PURE__ */ new aO(), J = /* @__PURE__ */ new Set([
  "Script",
  "Body",
  "FunctionDefinition",
  "ClassDefinition",
...
function A(O, e) {
  ...
  return e.cursor(nO.IncludeAnonymous).iterate((t) => {
    if (t.name) {
      let o = Pe[t.name];
      if (o && o(t, Q, r) || !r && J.has(t.name))
        return !1;
      r = !1;
    } else if (t.to - t.from > 8192) {
      for (let o of A(O, t.node))
        a.push(o);
      return !1;
    }
  }), V.set(e, a), a;
}
const G = /^[\w\xa1-\uffff][\w\d\xa1-\uffff]*$/, N = ["String", "FormatString", "Comment", "PropertyName"];
function me(O) {
  let e = K(O.state).resolveInner(O.pos, -1);
  if (N.indexOf(e.name) > -1)
    return null;
  let i = e.name == "VariableName" || e.to - e.from < 20 && G.test(O.state.sliceDoc(e.from, e.to));
  if (!i && !O.explicit)
    return null;
  let a = [];
...
const $e = /* @__PURE__ */ [
  ...
  "vars",
  "zip"
].map((O) => ({ label: O, type: "function" }))), ce = [
...
], he = /* @__PURE__ */ H(N, /* @__PURE__ */ rO(/* @__PURE__ */ $e.concat(ce)));
function z(O) {
...
function ye() {
  return new M(R, [
    R.data.of({ autocomplete: me }),
    R.data.of({ autocomplete: he })
  ]);
}
export {
  he as globalCompletion,
