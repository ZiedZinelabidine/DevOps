import { L as be, E as $, C as Pe } from "./index-DPyK7ojY.js";
import { p as xe, s as Te, t as h, y as Ve, L as we, m as K, E as ye, a as Xe, i as ve, f as ke, z as $e } from "./Header-DaHJSYVl.js";
import { cssLanguage as J, css as _e } from "./index-CMVEj-k-.js";
import { typescriptLanguage as qe, jsxLanguage as Ce, tsxLanguage as Qe, javascriptLanguage as R, javascript as Ae } from "./index-8qjcNtaV.js";
const Ye = 54, Me = 1, Re = 55, Ee = 2, Ze = 56, Be = 3, z = 4, ze = 5, v = 6, ee = 7, te = 8, ae = 9, le = 10, We = 11, De = 12, Ne = 13, _ = 57, Ge = 14, W = 58, re = 20, Ie = 22, ne = 23, je = 24, Y = 26, se = 27, Ue = 28, Le = 31, Fe = 34, He = 36, Ke = 37, Je = 0, et = 1, tt = {
  area: !0,
  base: !0,
  br: !0,
  col: !0,
...
const ot = [v, le, ee, te, ae], ut = new Pe({
  start: null,
  shift(e, a, r, l) {
  ...
  reuse(e, a, r, l) {
    let t = a.type.id;
    return t == v || t == He ? new j(M(l, 1) || "", e) : e;
  },
  strict: !1
}), Ot = new $((e, a) => {
  if (e.next != ue) {
  ...
  let l = M(e, 0);
  if (l === void 0) return;
  if (!l) return e.acceptToken(r ? Ge : v);
  ...
  if (r) {
    ...
    t && D[t] && D[t][l] ? e.acceptToken(_, -1) : e.acceptToken(v);
  }
...
const pt = Z("script", Ye, Me), ft = Z("style", Re, Ee), mt = Z("textarea", Ze, Be), St = Te({
  ...
  tokenizers: [pt, ft, mt, dt, Ot, it, 0, 1, 2, 3, 4, 5],
...
function ie(e = [], a = []) {
  ...
  return xe((u, O) => {
    let m = u.type.id;
    if (m == Ue) return q(u, O, r);
    if (m == Le) return q(u, O, l);
    if (m == Fe) return q(u, O, t);
    if (m == re && s.length) {
      let p = u.node, c = p.firstChild, f = c && U(c, O), i;
      if (f) {
        for (let d of s)
          if (d.tag == f && (!d.attrs || d.attrs(i || (i = Oe(c, O))))) {
...
function Pt(e, a, r, l, t) {
  ...
  return {
    from: l,
    to: t,
    options: O.map((m) => ({ label: m, type: "property" })),
    validFor: fe
  };
}
function xt(e, a, r, l, t) {
  ...
  if (o) {
    let m = e.sliceDoc(o.from, o.to), p = a.globalAttrs[m];
    if (!p) {
      let c = T(r), f = c ? a.tags[x(e.doc, c)] : null;
      p = f?.attrs && f.attrs[m];
    }
    if (p) {
      let c = e.sliceDoc(l, t).toLowerCase(), f = '"', i = '"';
      /^['"]/.test(c) ? (O = c[0] == '"' ? /^[^"]*$/ : /^[^']*$/, f = "", i = e.sliceDoc(t, t + 1) == c[0] ? "" : c[0], c = c.slice(1), l++) : O = /^[^\s<>='"]*$/;
      for (let d of p)
        u.push({ label: d, apply: f + d + i, type: "constant" });
    }
  }
...
function _t(e = {}) {
  ...
  let l = r ? he.configure({ wrap: r, dialect: a }) : a ? X.configure({ dialect: a }) : X;
  return new Xe(l, [
    X.data.of({ autocomplete: Tt(e) }),
...
const H = /* @__PURE__ */ new Set(/* @__PURE__ */ "area base br col command embed frame hr img input keygen link meta param source track wbr menuitem".split(" ")), wt = /* @__PURE__ */ Ve.inputHandler.of((e, a, r, l, t) => {
  if (e.composing || e.state.readOnly || a != r || l != ">" && l != "/" || !X.isActiveAt(e.state, a, -1))
    return !1;
  let s = t(), { state: o } = s, u = o.changeByRange((O) => {
    var m, p, c;
    let f = o.doc.sliceString(O.from - 1, O.to) == l, { head: i } = O, d = K(o).resolveInner(i, -1), g;
    if (f && l == ">" && d.name == "EndTag") {
      let b = d.parent;
      if (((p = (m = b.parent) === null || m === void 0 ? void 0 : m.lastChild) === null || p === void 0 ? void 0 : p.name) != "CloseTag" && (g = x(o.doc, b.parent, i)) && !H.has(g)) {
        let P = i + (o.doc.sliceString(i, i + 1) === ">" ? 1 : 0), y = `</${g}>`;
        return { range: O, changes: { from: i, to: P, insert: y } };
      }
    } else if (f && l == "/" && d.name == "IncompleteCloseTag") {
      let b = d.parent;
