import { L as X, E as m, a as v } from "./index-DPyK7ojY.js";
import { s as z, t as o, L as x, a as W, m as T, i as U, f as Z, g as R, b as Y, J as _, I as E } from "./Header-DaHJSYVl.js";
const q = 100, S = 1, G = 101, C = 102, h = 2, b = [
  9,
  10,
  11,
  12,
  ...
  8287,
  12288
], V = 58, N = 40, P = 95, D = 91, p = 45, I = 46, F = 35, j = 37, B = 38, K = 92, L = 10;
function d(e) {
...
const A = new m((e, t) => {
  for (let l = !1, a = 0, O = 0; ; O++) {
    let { next: r } = e;
    if (d(r) || r == p || r == P || l && w(r))
      !l && (r != p || O > 0) && (l = !0), a === O && r == p && a++, e.advance();
    else if (r == K && e.peek(1) != L)
      e.advance(), e.next > -1 && e.advance(), l = !0;
    else {
      l && e.acceptToken(r == N ? G : a == 2 && t.canShift(h) ? h : C);
      break;
    }
  }
}), J = new m((e) => {
  if (b.includes(e.peek(-1))) {
    let { next: t } = e;
    (d(t) || t == P || t == F || t == I || t == D || t == V && d(e.peek(1)) || t == p || t == B) && e.acceptToken(q);
  }
}), H = new m((e) => {
  if (!b.includes(e.peek(-1))) {
    let { next: t } = e;
    if (t == j && (e.advance(), e.acceptToken(S)), d(t)) {
      do
        e.advance();
      while (d(e.next) || w(e.next));
      e.acceptToken(S);
    }
  }
}), M = z({
  ...
  tokenizers: [J, H, A, 1, 2, 3, 4, new v("m~RRYZ[z{a~~g~aO#_~~dP!P!Qg~lO#`~~", 28, 106)],
  topRules: { StyleSheet: [0, 4], Styles: [1, 87] },
  specialized: [{ term: 101, get: (e) => ee[e] || -1 }, { term: 59, get: (e) => Oe[e] || -1 }, { term: 102, get: (e) => ae[e] || -1 }],
  tokenPrec: 1219
});
let Q = null;
function u() {
...
const g = /* @__PURE__ */ [
  ...
  "visited",
  "where"
].map((e) => ({ type: "class", label: e })), f = /* @__PURE__ */ [
  "above",
  "absolute",
  "activeborder",
  "additive",
...
const y = /* @__PURE__ */ new _(), ne = ["Declaration"];
function se(e) {
  for (let t = e; ; ) {
    if (t.type.isTop)
      return t;
...
const de = (e) => (t) => {
  let { state: l, pos: a } = t, O = T(l).resolveInner(a, -1), r = O.type.isError && O.from == O.to - 1 && l.doc.sliceString(O.from, O.to) == "-";
  ...
  if (O.name == "PseudoClassName")
    return { from: O.from, options: g, validFor: s };
  ...
  return n && n.name == ":" && i.name == "PseudoClassSelector" ? { from: a, options: g, validFor: s } : n && n.name == ":" && i.name == "Declaration" || i.name == "ArgList" ? { from: a, options: f, validFor: s } : i.name == "Block" || i.name == "Styles" ? { from: a, options: u(), validFor: s } : null;
}, ce = /* @__PURE__ */ de((e) => e.name == "VariableName"), $ = /* @__PURE__ */ x.define({
  name: "css",
...
function ue() {
  return new W($, $.data.of({ autocomplete: ce }));
}
export {
  ue as css,
