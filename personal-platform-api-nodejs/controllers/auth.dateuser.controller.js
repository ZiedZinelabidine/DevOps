...
const { validateSignupUser } = require("../validators/dateuser.validators");
...
exports.signup = async (req, res) => {
  ...
  try {
      ...
      const isEmailUnique = await  isFieldValueUniqueDateUser("email", userData.email) ;
    ...
    // Create a Stripe customer
      ...
      const user = await Dateuser.create(userData, { transaction });

      // Create and return authentication token
      const token = await createTokenDateUser(user, "DATEUSER");
...
exports.forgetPasswordUser = async (req, res) => {
  try {
    ...
    const user = await Dateuser.findOne({ where: { email } });
    ...
    await user.save();
    return res.status(200).json({ message:  "resest password token was generated ",resetToken , type: "DATEUSER"});
...
exports.ResetVerificationTokenUser = async (req, res) => {
  try {
    ...
    const token = await createTokenDateUser(user, "DATE_USER");
