...
#   name                = "pip-aks-${var.application_name}-${var.environment_name}-outbound"
...
#   name       = "rg-${var.application_name}-${var.environment_name}-nodes"
#   location   = local.location
#   managed_by = "/subscriptions/957c5f98-8ae3-465e-9031-87e4165a548c/resourcegroups/aks_tf_rg/providers/Microsoft.ContainerService/managedClusters/aks-itgalaxy-production"
#   tags = {
#     "aks-managed-cluster-name" = "aks-itgalaxy-production"
#     "aks-managed-cluster-rg"   = "aks_tf_rg"
#   }
# }
...
resource "azurerm_user_assigned_identity" "aks_cluster" {
  location            = local.location
  resource_group_name = local.resource_group_name
  name                = "mi-aks-${var.application_name}-${var.environment_name}"
}
...
resource "azurerm_kubernetes_cluster" "main" {
  ...
  location                          = local.location
  resource_group_name               = local.resource_group_name
  node_resource_group               = "rg-${var.application_name}-${var.environment_name}-nodes"
  sku_tier                          = "Standard"
  ...
  default_node_pool {
    ...
    zones          = [1]
    # Add this parameter during updates
    temporary_name_for_rotation = "rotationv3"
  }
...
resource "azurerm_kubernetes_cluster_node_pool" "workload" {
  ...
  # Add this parameter during updates
  temporary_name_for_rotation = "rotationv4"
