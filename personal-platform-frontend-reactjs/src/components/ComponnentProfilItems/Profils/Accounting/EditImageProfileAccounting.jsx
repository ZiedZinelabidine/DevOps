...
const EditImageProfilAccounting = ({
  data,
  ...
  const fetchImageUrl = async () => {
    try {
      const url = await dispatch(
        getURL({
          location: `accountings/${data?.id}/profil/img-profil/`,
        })
      );
    ...
    if (file) {
      setImageUrl(
        process.env.REACT_APP_URL_BUCKET +
          `accountings/${data?.id}/profil/img-profil/` +
          file.name
      );
...
export default EditImageProfilAccounting;
