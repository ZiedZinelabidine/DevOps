...
import GoogleAuthCandidat from "components/GoogleAuth/Signup/GoogleAuthCandidat";

const FreeLanceForm = ({ recruitment_token, proxy, handleModalRegister }) => {
  ...
  const SignUp = async () => {
    try {
      ...
      const usernametinode = handleGenerate(
        "candidat" + name.replace(/\s+/g, '') + first_name.replace(/\s+/g, '') + timestamps
      ).toString();
  ...
  return (
    <StyleContentModal> 
      <hr />
      <GoogleAuthCandidat recruitment_token={recruitment_token} />
        ...
        <FormGroupRegisterTwoCol style={{ paddingRight: "20px" }}>
          ...
          <input
            ...
            onChange={(e) => {
             ...
             // setIsNameValid(!/\s/.test(e.target.value)); // Check for spaces
            }}
            required
          />
          {!isNameValid && (
        ...
        <FormGroupRegisterTwoCol>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              //setIsFirstNameValid(!/\s/.test(e.target.value)); // Check for spaces in first name
            }}
            required
          />
          {!isFirstNameValid && (
